...
log_invalid_input_reason() {
  doLog "This might be caused by the command is not running on the master Getaway node or there has never had a backup/restore from the CDP CLI run."
}
...
while getopts ":s:h:p:u:g:n:" OPTION; do
    ...
    n  )
        DB_NAME+=("$OPTARG")
        DB_NAME_ITEM="$OPTARG"
        doLog "Names of the databases to restore. If not given, will restore ranger and hive databases: $DB_NAME_ITEM"
        ;;
    \? ) doLog "Unknown option: -$OPTARG" >&2;
        exit 1;;
