...
public class EC2ClientActions extends EC2Client {
    ...
    public Map<String, String> listLaunchTemplatesUserData(String stack) {
        ...
        try (Ec2Client ec2Client = buildEC2Client()) {
            ...
            DescribeLaunchTemplatesResponse launchTemplates = ec2Client.describeLaunchTemplates(DescribeLaunchTemplatesRequest.builder()
                    .launchTemplateIds(launchTemplateList.stream().map(StackResourceSummary::physicalResourceId).collect(Collectors.toList()))
                    .build());
            for (int i = 0; i < launchTemplateList.size(); i++) {
                DescribeLaunchTemplateVersionsResponse ver = ec2Client.describeLaunchTemplateVersions(DescribeLaunchTemplateVersionsRequest.builder()
                ...
                response.put(launchTemplates.launchTemplates().get(i).launchTemplateId(), userData);
            }
